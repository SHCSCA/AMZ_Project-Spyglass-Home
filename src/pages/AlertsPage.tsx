...
async function fetchAlerts(page: number, size: number, type?: string): Promise<PageResponse<AlertLogResponse>> {
  const q = new URLSearchParams({ page: String(page), size: String(size) });
  if (type) q.set('type', type);
  return apiRequest<PageResponse<AlertLogResponse>>(`/api/alerts?${q.toString()}`);
}
...
const AlertsPage: React.FC = () => {
  ...
  const pageSize = 20;
  const { data, loading, error, reload } = useFetch(() => fetchAlerts(page - 1, pageSize, alertType), [page, alertType]);
    ...
    <div>
      <Space style={{ marginBottom: 12 }}>
        <Select
          ...
          style={{ width: 180 }}
          value={alertType}
          onChange={(v) => { setPage(1); setAlertType(v); }}
          options={[
          ...
          <List.Item>
            <Card size="small" hoverable onClick={() => navigate(`/asin/${item.asinId}`)}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <div><strong>{item.asin}</strong> · {item.type} {item.severity && <Tag color={item.severity === 'ERROR' ? 'red' : item.severity === 'WARN' ? 'orange' : 'blue'}>{item.severity}</Tag>}</div>
