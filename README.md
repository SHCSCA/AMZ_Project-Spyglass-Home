...
<h1>🔍 Spyglass - 亚马逊竞品监控系统</h1>
<p>React + TypeScript + Ant Design + ECharts 打造的专业电商监控平台</p>
...
Spyglass 是一款专为亚马逊卖家设计的竞品监控系统，帮助您实时追踪商品价格、排名、库存、内容变化等关键指标，快速响应市场变化。当前 README 已更新至最新生产迭代（详情页最新快照逻辑修复、冗余代码清理、返回导航按钮新增）。

### 核心功能

- 📊 **ASIN 仪表盘** - 批量管理监控任务，支持分组筛选，快速查看关键指标
- 📈 **数据可视化** - 价格/BSR/库存趋势图表，直观展示历史变化
- 🚨 **智能告警** - 自动监测异常变化，按日期分组展示告警记录
- 📝 **详情分析** - 完整的商品历史数据表格，支持内容差异对比
- 🏷️ **分组管理** - 灵活的商品分组功能，提升管理效率
- 🔁 **最新快照合并** - 详情页自动合并 `/api/asin/by-asin/{asin}` 返回快照与历史列表，按 `snapshotAt` 选真正最新点
- ↩️ **快捷返回** - 详情页头部新增“返回上一页”按钮，提升导航效率
- 🧪 **稳定测试** - 增加针对逆序历史与较新快照的测试用例，保证最新值选择正确
- 🧹 **生产清理** - 删除未使用的 `fetchLatestSnapshot`，减少冗余，降低维护成本

### 最近变更摘要 (Changelog Highlights)

| 日期 | 类型 | 描述 |
| ---- | ---- | ---- |
| 2025-11-11 | fix | 修复详情页可能显示“最早”而非“最新”历史点的问题（排序 + 合并快照） |
| 2025-11-11 | chore | 移除未使用函数 `fetchLatestSnapshot`，优化测试结构与注释 |
| 2025-11-11 | feat | ASIN 详情页新增返回上一页按钮 |

> 若需查看更细粒度提交，请使用：`git log --since=2025-11-10`。
...
访问：`http://your-server:8082`

### 健康检查与日志

- 运行 `./scripts/verify-api.sh` 确认后端关键端点可访问
- 前端内置内存日志查看器 (页面右上角“系统日志”)，包含：请求成功/失败、重试、缓存命中、最新快照计算日志
- 若详情页数据异常，可在 URL 添加 `?debugCharts` 以快速禁用图表验证其它数据是否正常
